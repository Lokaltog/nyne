...
/// `/proc/<pid>/ns/user`
pub(super) fn ns_user(pid: Pid) -> PathBuf { proc_pid(pid).join("ns/user") }

/// `/proc/<pid>/ns/mnt`
pub(super) fn ns_mnt(pid: Pid) -> PathBuf { proc_pid(pid).join("ns/mnt") }
...
pub(super) const NYNE_BIN_DIR: &str = "/nyne/bin";

/// Per-PID subdirectory under the persist root for overlay upperdirs.
pub(super) fn persist_slot(persist_root: &Path, pid: Pid) -> PathBuf { persist_root.join(format!("upper-{pid}")) }

/// Default persist root under XDG cache: `$XDG_CACHE_HOME/nyne/overlay/`.
pub(super) fn default_persist_root(cache_dir: &Path) -> PathBuf { cache_dir.join("nyne/overlay") }
...
pub(super) fn lower_base(target_dir: &Path, pid: Pid) -> PathBuf { target_dir.join(format!("nyne/lower-{pid}")) }
...
pub(super) fn merged_base(pid: Pid) -> PathBuf { PathBuf::from(format!("/tmp/nyne-merged-{pid}")) }

...
///
/// Holds the generated byte content alongside provenance metadata
/// needed for staleness checks and per-provider invalidation.
struct ContentEntry {
    /// The generated content, shared via `Arc` so reads can return
    /// a clone-free handle without holding the cache lock.
    data: Arc<[u8]>,
    /// Which provider produced this content — enables bulk invalidation
    /// when a provider is deactivated or reloaded.
    provider_id: ProviderId,
    /// Source file and its [`FileGenerations`] counter at cache time.
    ///
    /// `None` for entries not derived from a specific source file
    /// (e.g., purely synthetic content). When present, [`ContentCache::is_stale`]
    /// compares this against the current generation to detect modifications.

...
### Parametrized tests are the default

**Before writing any test, think through the cases first.** List the input/expected-output pairs, then decide the structure:

1. **2+ cases with identical structure** → `#[rstest]` with `#[case]`. This is the overwhelmingly common case.
2. **Truly unique one-off assertion** → plain `#[test]`. Rare.
3. **Invariant property over random inputs** → `proptest!`.

`#[rstest]` is not an optimization — it's the default test style. Reach for it first, not as a refactoring afterthought. Name each case with `#[case::descriptive_name(...)]` so failures identify which scenario broke.

```rust
// GOOD: parametrized — easy to add cases, failures identify the scenario
#[rstest]
#[case::read_only(true, false, Permissions::READ)]
#[case::write_only(false, true, Permissions::WRITE)]
#[case::read_write(true, true, Permissions::READ | Permissions::WRITE)]
#[case::no_capabilities(false, false, Permissions::NONE)]
fn file_permissions_from_capabilities(
    #[case] readable: bool,
    #[case] writable: bool,
    #[case] expected: Permissions,
) { /* ... */ }

// BAD: four separate tests with identical structure
#[test] fn file_read_only_permissions() { /* ... */ }
#[test] fn file_write_only_permissions() { /* ... */ }
#[test] fn file_read_write_permissions() { /* ... */ }
#[test] fn file_no_capabilities_permissions() { /* ... */ }
```

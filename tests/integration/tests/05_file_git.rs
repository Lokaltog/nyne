...
#[rstest]
fn t_403_notes(mount: NyneMount) { mount.sh_ok(&format!("cat {FILE}@/git/NOTES.md")); }

/// T-404: `git/BLAME.md:M-N` — range-sliced blame is non-empty.
#[rstest]
fn t_404_blame_range(mount: NyneMount) {
    assert!(
        !mount.sh_ok(&format!("cat {FILE}@/git/BLAME.md:1-3")).trim().is_empty(),
        "expected blame entries in range 1-3",
    );
}

/// T-405: `git/LOG.md:-N` — last N log entries.
#[rstest]
fn t_405_log_last_n(mount: NyneMount) { mount.sh_ok(&format!("cat {FILE}@/git/LOG.md:-5")); }

/// T-406: `git/LOG.md:M-N` — range-sliced log.
#[rstest]
fn t_406_log_range(mount: NyneMount) { mount.sh_ok(&format!("cat {FILE}@/git/LOG.md:1-3")); }
...
fn t_409_history(mount: NyneMount) {
    assert!(
        !mount.sh_ok(&format!("ls {FILE}@/history/")).trim().is_empty(),
        "expected at least one historical version"
    );

    // Read the first historical version — should be non-empty Rust source.
    assert_contains_any(
        &mount.sh_ok(&format!("cat {FILE}@/history/$(ls {FILE}@/history/ | head -1)")),
        &["use ", "fn ", "struct ", "mod ", "impl "],
    );
}

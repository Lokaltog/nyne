    ...
    fn render(&self, engine: &TemplateEngine, template: &str) -> Result<Vec<u8>> {
        ...
        let analysis = {
            let hints: Vec<Hint> = shared
                .tree
                .as_ref()
                .map(|tree| self.engine.analyze(tree, &shared.source))
                .unwrap_or_default();
            build_view(&hints)
        };
...
/// Categorized analysis output ready for template rendering.
struct AnalysisView {
    /// Individual hint rows for low-frequency rules.
    hints: Vec<HintView>,
    /// Summary groups for rules above the collapse threshold.
    collapsed: Vec<CollapsedGroup>,
    /// Deduplicated suggestion rows across all hints.
    suggestions: Vec<SuggestionRow>,
}

fn build_view(hints: &[Hint]) -> AnalysisView {

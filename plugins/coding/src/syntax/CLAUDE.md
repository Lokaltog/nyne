...
- `merge_preceding_sibling_ranges` and `collect_import_range` in `syntax/parser.rs` trim trailing newlines from merged ranges. Any new range-merging code must do the same — otherwise splicing at the range will eat the `\n` separator between the collected content and the following symbol.

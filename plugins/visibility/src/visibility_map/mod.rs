...
impl VisibilityMap {
    ...
    /// 2. Name-based rule via `/proc/{pid}/comm` — the process's own
    ///    identity takes priority over inherited cgroup/ancestor visibility.
    ///    This ensures passthrough processes (e.g. git) are always hidden
    ///    regardless of which cgroup they inherit from their parent shell.
    /// 3. Cgroup membership — if cgroups v2 tracking is active, check whether
    ...
    fn resolve_by_comm(&self, pid: u32) -> Option<ProcessVisibility> {
        ...
        self.dynamic_name_rules
            ...
            .get(comm.as_str())
            .copied()
            .or_else(|| self.name_rules.get(comm.as_str()).copied())
    }
}

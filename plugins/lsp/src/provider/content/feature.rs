...
impl Feature {
    ...
    pub(crate) fn file_name(self) -> &'static str { &self.metadata().file_name }

    /// Symlink directory name for this feature, if it has one.
    pub(crate) fn dir_name(self) -> Option<&'static str> { self.metadata().dir_name.as_deref() }
    ...
    pub(crate) fn register_globals(engine: &mut TemplateEngine) {
    ...
    pub(crate) fn from_dir_name(name: &str) -> Option<Self> { Self::iter().find(|f| f.dir_name() == Some(name)) }
...
/// A resolved LSP result target for symlink directory population.
///
/// Produced by [`QueryResult::into_targets`] with paths already relativized
/// (overlay root stripped). Consumed by
/// [`build_target_nodes`](crate::provider::lsp_links)
/// to create symlink nodes pointing at the target symbol's body file.
pub struct Target {
    /// Project-relative file path (overlay root already stripped).

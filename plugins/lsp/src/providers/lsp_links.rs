...
use crate::providers::content::{LspTarget, query_lsp_targets};
use crate::providers::content::actions;
use nyne_source::providers::names::{COMPANION_SUFFIX, SUBDIR_SYMBOLS, companion_name};
use nyne_source::providers::syntax::SyntaxProvider;
use nyne_source::services::SourceServices as CodingServices;
...
#[allow(dead_code)]
pub(crate) fn resolve_lsp_symlink_dir(
    provider: &SyntaxProvider,
    source_file: &VfsPath,
    ...
    let Some(dctx) = provider.decomposition_context(source_file)? else {
        return Ok(None);
    };
    ...
    let Some(lsp_handle) = LspHandle::for_file(&provider.ctx, source_file) else {
        return Ok(None);
    };
    ...
    let root = provider.ctx.root();
    let base = build_symlink_base(source_file, fragment_path, lsp_dir)?;
    let nodes = build_target_nodes(provider, &targets, root, &base);
    Ok(Some(nodes))
}
...
fn build_target_nodes(provider: &SyntaxProvider, targets: &[LspTarget], root: &Path, base: &VfsPath) -> Vec<VirtualNode> {
    ...
    for target in targets {
        ...
        let symlink_target = resolve_symbol_link(provider, &target_vfs, rel_path, target.line, base)
